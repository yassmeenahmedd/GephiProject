...
def _build_color_map(G: nx.Graph, color_attr: Optional[str]) -> Dict[Any, str]:
    ...
    return {
        v: QUALITATIVE_COLORS[i % len(QUALITATIVE_COLORS)]
...
def visualize_pyvis(
    ...
    color_attr: Optional[str] = None,          # ← CHANGED from "Class" to None
    ...
    # Build color map
    color_map = _build_color_map(G, color_attr)
    ...
    for i, (node_id, data) in enumerate(nodes_list):
        ...
        # ✅ COLOR FIX
        if color_attr and color_attr != "None":
